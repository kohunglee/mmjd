...
function widget_tag($title) {
    ...
    <div class="widget shadow-theme">
        ...
        <div class="unstyle-li tag-container">
            ...
            <a target='_blank' href="<?= Url::tag($value['tagurl']) ?>" title="<?= $value['usenum'] ?> 篇文章" class='site-side-tags'><?= htmlspecialchars($value['tagname']) ?></a></span>
            <?php endforeach ?>
        </div>
    </div>
<?php } ?>
    ...
    <div class="widget shadow-theme">
        ...
        <ul class="unstyle-li log-classify-f">
            ...
            foreach ($sort_cache as $value):
                ...
                ?>
                <li>
                    <a target='_blank' href="<?= Url::sort($value['sid']) ?>" title="<?= $value["description"] ?>"><?= $value['sortname'] ?>
        ...
        <ul class="unstyle-li">
            ...
            foreach ($com_cache as $value):
                ...
                <li class="comment-info">
                    ...
                    <a target='_blank' href="<?= $url ?>" class="latest-com-content"><?= $value['content'] ?></a>
                    <hr>
                </li>
            <?php endforeach ?>
        </ul>
    ...
    <div class="widget shadow-theme">
        ...
        <ul class="unstyle-li">
            <?php foreach ($newLogs_cache as $value): ?>
                <li class="blog-lates"><a target='_blank' href="<?= Url::log($value['gid']) ?>"><?= $value['title'] ?></a></li>
            <?php endforeach ?>
        </ul>
    </div>
<?php } ?>
    ...
    <div class="widget shadow-theme">
        ...
        <ul class="unstyle-li">
            <?php foreach ($hotLogs as $value): ?>
                <li class="blog-hot"><a target='_blank' href="<?= Url::log($value['gid']) ?>"><?= $value['title'] ?></a></li>
            <?php endforeach ?>
        </ul>
    </div>
<?php } ?>
...
function widget_archive($title) {
    ...
    <div class="widget shadow-theme">
        ...
        <select id="site_archive" class="archive">
...
function editflg($logid, $author) {
    $editflg = User::haveEditPermission() || $author == UID ? '&nbsp;&nbsp;&nbsp;<a target="_blank" href="' . BLOG_URL . 'admin/article.php?action=edit&gid=' . $logid . '" target="_blank">编辑本文</a>' : '';
    echo $editflg;
}
...
function blog_sort($blogid) {
    ...
    <?php if (!empty($sortName)) { ?>
        <a target='_blank' href="<?= Url::sort($sortID) ?>"><?= $sortName ?></a>
    <?php } else { ?>
...
function blog_tag($blogid) {
    ...
    if (!empty($tag_names)) {
        ...
        foreach ($tag_names as $key => $value) {
            $tag .= "    <a target='_blank' href=\"" . Url::tag(rawurlencode($value)) . "\" class='tags' title='标签' >" . htmlspecialchars($value) . '</a>';
        }
        echo $tag;
    }
}
...
function blog_author($uid) {
    ...
    echo '<a target="_blank" href="' . Url::author($uid) . "\">$author</a>";
}
...
function blog_comments($comments) {
    ...
    foreach ($commentStacks as $cid):
        ...
        $comment['poster'] = $comment['url'] ? '<a target="_blank" href="' . $comment['url'] . '" rel="external nofollow" target="_blank">' . $comment['poster'] . '</a>' : $comment['poster'];
        ?>
    ...
    ?>
        ...
        foreach ($navi_cache as $value):
            ...
            if ($value['url'] == 'admin' && (!User::isVistor())):
                ?>
                    <a target='_blank' href="<?= BLOG_URL ?>admin/" class="site-nav-link">管理</a>
                    <a target='_blank' href="<?= BLOG_URL ?>admin/account.php?action=logout" class="site-nav-link">退出</a>
                <?php
                continue;
            endif;
            ...
            <?php if (!empty($value['children']) || !empty($value['childnavi'])) : ?>
                <?php if (!empty($value['children'])): ?>
                    <a class="site-nav-link has-down" id="nav_link" href="<?= $value['url'] ?>" <?= $newtab ?>><?= $value['naviname'] ?></a>
...
/**
 * 根据用户名，获取邮箱地址，并转化为 Gravatar 头像
 */
function mmjd_getAvatarImg($uid){
    $User_Model = new User_Model();
    $user_info = $User_Model->getOneUser($uid);
    $authorEmail = $user_info['email'];
    
    if(!empty($authorEmail)) {
        echo getGravatar($authorEmail);
    } else {
        global $CACHE;
        $user_cache = $CACHE->readCache('user');
        echo BLOG_URL . $user_cache[1]['photo']['src'];
    }
}